...
namespace MaisPDF
{
    public class MBarcodeQR : BarCode
	{
		...
		internal void Render(XGraphics gfx, XBrush brush, XFont font, XPoint position, string content, Encoding encoding)
		{
			...
			for (int i = 0; i < matrix.Length; i++)
			{
				for (int j = 0; j < matrix.Length; j++)
					...
					if (matrix[j][i])
					{
						gfx.DrawRectangle(brush, new XRect(position.X + (j * _scale), position.Y + (i * _scale), _scale, _scale));
					}
			}
		}
		...
		{
			if (QRCodeUtility.IsUnicode(Text))
			{
				Render(gfx, brush, font, position, Text, Encoding.Unicode);
			}
			else
			{
				Render(gfx, brush, font, position, Text, Encoding.ASCII);
			}
		}
	}
}

	public class QRBarcodeMetrics
	{
		public int Scale
		{
			get;
